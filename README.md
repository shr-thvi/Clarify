...
If Docker Desktop is installed but the daemon does not start, open Docker Desktop and ensure it is running before retrying `docker compose up --build`.

Docker Compose now includes a built-in health check against `/api/health` so the container can be monitored and restarted automatically when unhealthy.
