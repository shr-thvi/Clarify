...
export default function InteractiveSocraticChat({ 
  ...
  const buildFallbackTutorResponse = (studentText: string, subject: string) => {
    const cleaned = studentText.trim();
    const lower = cleaned.toLowerCase();
    const subjectName = subject || "this topic";
    const currentDraftQuery = cleaned.endsWith("?")
      ? cleaned
      : `I need help understanding ${cleaned} in ${subjectName}.`;

    if (lower.includes("ac")) {
      return {
        tutorReply: "Good start. When you say AC, do you mean alternating current in electricity, and are you asking why its direction changes periodically?",
        currentDraftQuery,
    ...
    if (lower.includes("formula") || lower.includes("equation")) {
      return {
        tutorReply: "Let us pin down the exact formula first. Which term in the formula feels unclear, and what values or conditions are given in your question?",
        currentDraftQuery,
    ...
    if (lower.includes("step") || lower.includes("solve") || lower.includes("calculation")) {
      return {
        tutorReply: "That sounds like a step-by-step confusion. Tell me the last step you understood and the first step where the solution stopped making sense.",
        currentDraftQuery,
        confidenceInDraft: 60,
        nextSuggestedInput: "I understood the first step, but not the next calculation."
      };
    }

    return {
      tutorReply: `I can help with that. What exactly is confusing you in ${subjectName}: the definition, the formula, the diagram, or the calculation step?`,
      currentDraftQuery,
      confidenceInDraft: Math.max(confidence, 45),
  ...
  const handleSendMessage = async (customMessage?: string) => {
    ...
    try {
      ...
      const url = `${window.location.origin}/api/clarify-chat`;
      console.debug('POST', url, { history: chatHistory.length, currentQuery: textToSend });
      ...
      const fallback = buildFallbackTutorResponse(textToSend, initialSubject);
      
      const tutorMsg: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: "tutor",
        content: data.tutorReply || data.assistantReply || fallback.tutorReply,
        draftQuery: data.currentDraftQuery || fallback.currentDraftQuery || draftQuery,
        confidenceInDraft: data.confidenceInDraft || fallback.confidenceInDraft,
        nextSuggestedInput: data.nextSuggestedInput || fallback.nextSuggestedInput
      };
      ...
      const errorMsg: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: "tutor",
        content: fallback.tutorReply,
        draftQuery: fallback.currentDraftQuery,
        confidenceInDraft: fallback.confidenceInDraft,
        nextSuggestedInput: fallback.nextSuggestedInput
      };
      setMessages((prev) => [...prev, errorMsg]);
      setDraftQuery(fallback.currentDraftQuery);
      setConfidence(fallback.confidenceInDraft);
    } finally {
      setLoading(false);
    }
  };
